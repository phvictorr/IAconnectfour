...
def minimax(board, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        ...
        for col in valid_locations:
            ...
            new_score = minimax(temp_board, depth - 1, alpha, beta, False)[1] #argumentos adicionados para poda alfa-beta
            if new_score > value:
                value = new_score
                column = col
            alpha = max(alpha, value)
            if alpha >= beta:
                break  # Poda alfa-beta nos ramos que não são necessários
        return column, value
...
while not game_over:
    ...
    # Movimento da IA
        ...
        inicio = time.time()
        col, minimax_score = minimax(board, 4, -np.Inf, np.Inf, True) #profundidade = 4 e argumentos adicionados para poda alfa-beta
        if valid_location(board, col):
